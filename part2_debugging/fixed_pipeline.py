...
FIX C — Replace the brittle exact-match regex with a robust number-extraction
         approach.  Fall back to asking the model again with a stricter prompt
         if no number is found.  Log a warning — never silently return None.
"""
...
def _call_with_timeout(prompt: str, timeout: int = LLM_TIMEOUT_SECONDS) -> str:
    ...
    if t.is_alive():
        raise TimeoutError(
            f"LLM call did not complete within {timeout}s"
        )
    if error_box[0]:
...
def generate_summary(context: dict, retries: int = LLM_MAX_RETRIES) -> str:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            wait = 2 ** attempt
            logger.warning(
                "LLM timeout on attempt %d/%d; retrying in %ds", attempt, retries, wait
            )
            time.sleep(wait)
...
# Matches any 1-3 digit integer in the response (handles preamble and variations)
