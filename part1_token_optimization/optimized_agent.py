...
Two concrete optimizations applied:
  ...
  OPTIMIZATION 1 — Retrieval-Augmented Generation (RAG) instead of full-corpus injection
  ─────────────────────────────────────────────────────────────────────────────────────────
  Instead of dumping all 5 documents (~33 500 tokens) into every system prompt,
...
# ---------------------------------------------------------------------------
CHUNK_SIZE_TOKENS = 512  # each chunk ≈ 512 tokens

# Simulate a retrieval result: only the 3 most relevant chunks returned
RETRIEVED_CHUNKS = """
[CHUNK — Company Policy Manual §4.2 Expense Reimbursement]
...
[CHUNK — Company Policy Manual §4.3 Travel & Entertainment]
...
[CHUNK — Company Policy Manual §4.1 General Expense Policy]
...
"""  # ≈ 180 tokens — vs 33 500 for the full corpus
...
Assistant: Per §4.3, domestic per-diem is $65/day for meals and incidentals.
Receipts required for amounts over $25.
"""  # ≈ 55 tokens — vs ~3 750 tokens for all 10 × 3 examples
...
RECENT_TURNS = [
    ...
    {"role": "assistant", "content": "GDPR requires 30-day retention and EU-West residency for EU customer data."},
]  # ≈ 40 tokens for the 2 recent turns
...
class OptimizedAgent:
    ...
    def build_system_prompt(self) -> str:
        ...
        """
        return f"""You are a helpful enterprise assistant.  Answer questions
...
{self.retrieval_chunks}"""
    ...
    def build_messages(self, user_query: str) -> list[dict]:
        ...
        messages: list[dict] = [
            ...
            {"role": "assistant", "content": f"[Conversation summary so far]: {self.rolling_summary}"},
        ]
